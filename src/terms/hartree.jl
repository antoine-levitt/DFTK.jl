...
function compute_poisson_green_coeffs(basis::PlaneWaveBasis{T}, scaling_factor;
    ...
    # Solving the Poisson equation ΔV = -4π ρ in Fourier space is multiplying elementwise
    # by the Coulomb kernel v_c(G).  For a fully periodic model `truncated_coulomb_fourier`
    # returns 4π/|G|² (with v_c(0)=0 for the compensating background); for non-periodic
    # models it returns the Rozzi et al. (2006) truncated kernel.  The same code path
    # handles both cases with no branching.
